...
namespace MedicalSystem.Anes.CustomProject
{
    ...
    public partial class TimePointBloodGasEditor : BaseView
    {
        ...
        private void LoadGridList()
        {
            ...
            string[] list = ExtendAppContext.Current.DefaultBloodGasItem.ToArray();
            ...
            if (list != null && list.Length > 0)
            {
                ...
                {
                    string name = text;
                    if (ExtendAppContext.Current.BloodGasItemDict.ContainsKey(text))
                    {
                        name = ExtendAppContext.Current.BloodGasItemDict[text];
                    }
        ...
        private void TimePointBloodGasEditor_Load(object sender, EventArgs e)
        {
            ...
            {
                List<MED_BLOOD_GAS_MASTER> bloodMaster = AnesInfoService.ClientInstance.GetBloodGasMasterListByID(_detailID);
                if (bloodMaster != null && bloodMaster.Count == 1)
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!AnesInfoService.ClientInstance.SaveBloodGasMaster(bloodGasMasterDataTable)) return;
            ...
            if (!AnesInfoService.ClientInstance.SaveBloodGasDetail(detailTable)) return;
