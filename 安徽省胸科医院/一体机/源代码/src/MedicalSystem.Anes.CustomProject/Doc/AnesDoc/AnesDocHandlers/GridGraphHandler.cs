            ...
            switch (ApplicationConfiguration.PassedInLiquidInBloodFormat)
            {
                ...
                case "BloodInThree":
                    ...
                    if (control.HasLiquid)
                    {
                        ...
                        while (control.Rows.Count < control.MaxRowCount - 3 - 2)//-3 为输血留三行，给出液留2行
                            control.Rows.Add(new MedGridGraphRow("", Color.Black));
                        while (control.Rows.Count > control.MaxRowCount - 3 - 2)//-3 为输血留三行
                            control.Rows.RemoveAt(control.Rows.Count - 1);
                    ...
                    // 排序，方便后续增删操作
                    //control.Sort();
                    while (control.Rows.Count < control.MaxRowCount - 2)//默认9
                        control.Rows.Add(new MedGridGraphRow("", Color.Black));
                    while (control.Rows.Count > control.MaxRowCount -2)//默认9
                        control.Rows.RemoveAt(control.Rows.Count - 1);
                    ...
                    // 行数不够用空行补充
                    while (control.Rows.Count < control.MaxRowCount - 2)
                    {
                        control.Rows.Add(new MedGridGraphRow("", Color.Black));
                    }
                ...
                case "BloodInOne":
                    ...
                    {
                        this.AddGraphRow(control, anesEvent, new AnesClassType[] { AnesClassType.InLiquid }, "", Color.Black);
                        while (control.Rows.Count < control.MaxRowCount - 1 - 2)// 一行留给输血，两行留给出液
                            control.Rows.Add(new MedGridGraphRow("", Color.Black));
                        while (control.Rows.Count > control.MaxRowCount - 1 - 2)//-3空三行
                            control.Rows.RemoveAt(control.Rows.Count - 1);
                    }
                    // 显示输血
                    while (control.Rows.Count < control.MaxRowCount - 1 - 2)//默认9
                        control.Rows.Add(new MedGridGraphRow("", Color.Black));
                    while (control.Rows.Count > control.MaxRowCount - 1 - 2)//默认9
                        control.Rows.RemoveAt(control.Rows.Count - 1);
                    this.AddGraphRow(control, anesEvent, new AnesClassType[] { AnesClassType.InBlood }, "输血制品", Color.Black);
                    if (control.Rows.Count > control.MaxRowCount - 2)// 留两行给出液
                    {
                        control.Rows.RemoveAt(control.Rows.Count - 1);
                    }
                    // 排序，方便后续增删操作
                                 ...
                                 Color rowColor)
        {
            ...
            if (anesEventTable != null && anesEventTable.Count > 0)
            {
                ...
                if (anesEventTable != null && anesEventTable.Count > 0)
                {
                    ...
                    List<MED_EVENT_DICT> eventDict = ApplicationModel.Instance.AllDictList.EventDictList;//ExtendAppContext.Current.CommDict.EventDict;
                    ...
                    if (_currentGraph.MouseTime > DateTime.MinValue)
                    {
                        ...
                        if (pt == null)
                        {
                            ...
                            if (_currentGraph.Rows[Line - 1].Points.Count == 0 || Line == 7 || Line == 8 || Line == 9)
                            {
                                ...
                                DataTable sourceTable = new ModelHandler<MED_EVENT_DICT>().FillDataTable(eventDict);
                            ...
                            if (dialogHostForm.ShowDialog() == DialogResult.OK)
                            {
                                if (editItem != null && editItem.IsDelete)
                                {
                                    if (MessageBoxFormPC.Show("是否确定要删除选中事件？", "操作提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                    {
                                        ...
                                        List<MED_ANESTHESIA_EVENT> rows = allEvent.Where(x => x.EVENT_CLASS_CODE == row.EVENT_CLASS_CODE &&
                                                                                              x.ITEM_NO == row.ITEM_NO).ToList();
                                        if (rows.Count > 0)
                                        {
