...
using MedicalSystem.AnesWorkStation.Service;
...
namespace MedicalSystem.Anes.Client.CustomProject
{
    public partial class EmergencyRegisterDoc : CustomBaseDoc
    {
        ...
        DateTime serverTime = CommonService.ProxyInstance.GetServerTime();
        ...
        private void SyncPatientByPatientId(string patientID)
        {
            if (!string.IsNullOrEmpty(patientID))
            {
                ...
                {
                    string ret = "";
                    ret = new SyncInfoService().SyncPatientInfoAndInHospital(patientID);
                    ret += new SyncInfoService().SyncScheduleInfo(patientID, serverTime);
            ...
            if (patientID != "")
            {
                ...
                List<MED_PAT_VISIT> patVisitList = OperationInfoService.GetPatVisitList(patientID);
