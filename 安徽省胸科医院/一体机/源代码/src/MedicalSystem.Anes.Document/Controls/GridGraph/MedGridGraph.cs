    ...
    public partial class MedGridGraph : AnesGraph, IPrintable
    {
        ...
        private void DrawRows(Graphics g)
        {
            ...
            for (int i = 0; i < _rows.Count; i++)
            {
                ...
                this.DrawTotal(g, i, rowHeight);
            }
        }

        private double GetCurveTotal(MedGridGraphRow curve)
        {
            double totalValue = 0;
            foreach (MedGridPoint point in curve.Points)
            {
                if (point.Time >= _startTime && point.EndTime < _endTime)
                {
        ...
        private void DrawTotal(Graphics g, int index, float rowHeight)
        {
            MedGridGraphRow curve = _rows[index];
            Rectangle rect;
            Font font;
            SolidBrush solidBrush = new SolidBrush(this._singleTextColor);
            switch (_totalType)
            {
                case TotalType.SummeryValue:
                    ...
                    if (totalValue > 0)
                    {
                        g.DrawString(totalValue.ToString(), font, solidBrush, rect.X + rect.Width * (1 - _rightWidthPercent), rect.Top + index * (rowHeight + 1) + _topOffSet);
                    }
                    break;
