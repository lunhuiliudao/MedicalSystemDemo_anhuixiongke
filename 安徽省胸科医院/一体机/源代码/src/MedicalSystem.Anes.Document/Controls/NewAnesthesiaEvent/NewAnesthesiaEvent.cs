...
{
    public class NewAnesthesiaEvent
    {
        ...
        public static bool SaveEventDict(string itemClass, string itemNo)
        {
            List<MED_EVENT_DICT> eventDictList = DictService.ClientInstance.GetEventDictList();
            List<MED_EVENT_DICT_EXT> extList = DictService.ClientInstance.GetEventDictExtList();
            ...
            {
                DictService.ClientInstance.DelEventDict(eventDict);
            }
            foreach (MED_EVENT_DICT_EXT extDict in extList)
            {
                DictService.ClientInstance.DelEventExtDict(extDict);
            }
            return true;
        }
        public static void SaveEventExtDict(DataTable dataTable, int dosageCount)
        {
            List<MED_EVENT_DICT> eventDictList = DictService.ClientInstance.GetEventDictList();
            List<MED_EVENT_DICT_EXT> extList = DictService.ClientInstance.GetEventDictExtList();
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                ...
                DictService.ClientInstance.SaveEventExtList(extList);
            }
        }
        public static DataTable RefreshEventDictExt(string itemClass, string eventType, int dosageCount)
        {
            ...
            List<MED_EVENT_DICT> eventDictList = DictService.ClientInstance.GetEventDictList().FindAll(x => x.EVENT_CLASS_CODE == itemClass);
            if (!string.IsNullOrEmpty(eventType))
            {
                eventDictList = DictService.ClientInstance.GetEventDictList().FindAll(x => x.EVENT_CLASS_CODE == itemClass && x.EVENT_ATTR2 == eventType);
            }
            List<MED_EVENT_DICT_EXT> eventDictExtList = DictService.ClientInstance.GetEventDictExtList().FindAll(x => x.EVENT_CLASS_CODE == itemClass);
            foreach (MED_EVENT_DICT eventDict in eventDictList)
            {
