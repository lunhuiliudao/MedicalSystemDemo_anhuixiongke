...
namespace MedicalSystem.Anes.Document.Controls
{
    public class NewMonitorData
    {
        ...
        public bool Delete(string itemCode)
        {
            bool result = false;
            DateTime dtServer = DateTime.Now;
            List<MED_PAT_MONITOR_DATA_EXT> patMonitorDataExtList = AnesInfoService.ClientInstance.GetPatMonitorExtListByEvent(_patientID, _visitID, _operID, _eventNo).Where(x => x.ITEM_CODE == itemCode).ToList();
            List<MED_PAT_MONITOR_DATA> patMonitorDataList = AnesInfoService.ClientInstance.GetPatMonitorDataListByEvent(_patientID, _visitID, _operID, _eventNo).Where(x => x.ITEM_CODE == itemCode).ToList();
            if (patMonitorDataList != null && patMonitorDataList.Count > 0)
            {
                patMonitorDataList.ForEach(row =>
                {
