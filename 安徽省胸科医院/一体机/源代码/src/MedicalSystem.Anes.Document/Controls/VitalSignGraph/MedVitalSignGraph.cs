...
namespace MedicalSystem.Anes.Document.Controls
{
    ...
    public partial class MedVitalSignGraph : AnesGraph, IPrintable
    {
        ...
        private void DrawBloodGas(Graphics g)
        {
            ...
            // 注意：如果此页没有数字体征，血气会覆盖顶部刻度线，可以这样做：digitCount >= 0 ? digitCount + 1 : digitCount
            digitCount = digitCount > 0 ? digitCount + 1 : digitCount;// 血气不覆盖体征(自动向下移动一格)，前提：至少要有一个数字体征

            foreach (BloodGasMaster item in _bloodGasItems)
            {
                //left = GetX(item.Recorddate, rectF);
                left = GetBloodGas(item.Recorddate, rectF);// 麻醉单血气自动调整位置

                if (left.Equals(float.NaN))
                { continue; }
                top = rectF.Y + digitCount * g.MeasureString("A", new Font("宋体", 9)).Height;// 绘制血气的顶部位置
            ...
            using (Brush brush = new SolidBrush(_curves[index].Color))
            {
                ...
                if (_isDigitTop)
                {
                    ...
                    if (_curves[index].IsDigit && _curves[index].Points.Count > 0)
                    {
                        ...
                        {
                            g.DrawString(_curves[index].Text, font, brush, rectF.Left - g.MeasureString(_curves[index].Text, font).Width - 30, rectF.Top + idx * rowHeight);
                        }
                    }
        ...
        /// <summary>
        /// 计算血气的绘制位置
        /// 存在当血气的时间点和当前文书结束时间点相近会显示不完全，所以要单独处理
        /// </summary>
        private float GetBloodGas(DateTime timePoint, RectangleF rectF)
        {
            TimeSpan spThis, spTotal;
            spThis = timePoint - _startTime; // 血气的记录时间-文书开始时间，_startTime、_endTime从VitalSignGraphHandler的BindDataToUI方法里PagerSetting.PageTimeSpan.StartDateTime、EndDateTime获取
            spTotal = _endTime - _startTime; // 文书结束时间-文书开始时间
            if (spThis.Ticks < 0 || spTotal.Ticks <= 0 || spThis.TotalSeconds > spTotal.TotalSeconds)
            {
                // 等于5的情况在此处加上，补充了下面else里_endTime - timePoint<5的情况
                if (spThis.TotalMinutes >= -5.0 && spThis.TotalMinutes < 0)// spThis.TotalMinutes<0 前一页：_endTime - timePoint<5分钟，此时在下一页会进入此处
                {
                    spThis = _startTime - _startTime;// 0
                    return (float)(rectF.X + rectF.Width * (spThis.TotalSeconds / spTotal.TotalSeconds));// 左侧顶部书写 
                }
                return float.NaN;
            }
            else
            {
                // 如果当前时间点和结束时间靠近
                if ((_endTime - timePoint).TotalMinutes < 5) // 这个情况正常应该在下一页里出现，并且在上面那个if (spThis.TotalMinutes > -5.0)里
                {
                    return float.NaN;
                }
                else if ((_endTime - timePoint).TotalMinutes < 10)
                {
                    spThis = timePoint.AddMinutes(-20) - _startTime;
                }
                else if ((_endTime - timePoint).TotalMinutes <= 15)
                {
                    spThis = timePoint.AddMinutes(-10.0) - _startTime;
                }
        ...
        private MedVitalSignBreathControlTime DoBreathControl(MedVitalSignPoint point)
        {
            ...
            if (point.Curve.Text.Contains("呼吸"))
            {
                ...
                for (int i = 0; i < BreathControlTimeList.Count; i++)
                {
                    ...
                    if (point.Time >= breathControlTime.startTime && point.Time <= breathControlTime.endTime)
                    {
                    ...
                    if (d2 >= scaleValues[i].Value && d2 <= scaleValues[i + 1].Value)
                    {
                        ...
                        if (!_onlyShowFiveMinute || point.Time.Minute % 5 == 0)
                        {
                            ...
                            if (breathControlTime != null && breathControlTime.BreathType == BreathControlType.ControlBreath)//控制呼吸
                            {
                                ...
                                symbol.Text = "C";
                                //MedSymbol symbol = new MedSymbol(MedSymbolType.MachineAir);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!_printing && curve.CurveType == VitalSignCurveType.LineAndPoints && point.IsModify)
                                    {
                                        ...
                                        // modified by joysola on 2018-5-9 修复modifyPen报错的bug
                                        // curve.Symbol.Pen = ModifyPen;
                                        curve.Symbol.Pen = new Pen(Color.Red, 2f);
                                        curve.Symbol.Draw(g, point.X, point.Y);
                                        // modified end on 2018-5-9

                                        // modified by joysola on 2018-3-30 当 标记修改过的体征项 为 true时，画完修改过的体征点后，
                                        // symbolPen对象的内容会少了一部分，如：Alignment、Brush的缺失，
                                        // 原因 ：Symbol的Pen属性在调用set访问器的时候，会使用dipose()方法。。。。
                                        // 导致之后的点调用Draw（...,...,..）方法时报参数异常错误
                                        // 故 重新获取曲线的pen对象
                                        symbolPen = new Pen(curve.Color);
