        ...
        public void DrawGraphics(Graphics g, float x, float y)
        {
            ...
            foreach (Control ctl in Controls)
            {
                if (ctl is CheckBox)
                {
                    ...
                    else if (((int)left + xOffSet + g.MeasureString(ctl.Text, ctl.Font).Width) >= ((int)x + Width))
                    {
                        left = x;
                    ...
                    if ((ctl as CheckBox).Checked)
                    {
                        ...
                        g.DrawString(ctl.Text, ctl.Font, selectBrush, left + xOffSet + leftMargin, top);
                    }
                    else
                    {
                        g.DrawRectangle(pen, rect);
                        g.DrawString(ctl.Text, ctl.Font, solidBrush, left + xOffSet + leftMargin, top);
                    }
