...
namespace MedicalSystem.Anes.FrameWork
{
    public partial class ButtonColor : UserControl
    {
        ...
        private void panelBtn_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                gac.DrawString(this._title, new Font("微软雅黑", 12), new SolidBrush(Color.Gray), this.ClientRectangle, sf);
            }
            else
            {
                gac.DrawString(this._title, new Font("微软雅黑", 12), new SolidBrush(Color.White), this.ClientRectangle, sf);
            }
        }
