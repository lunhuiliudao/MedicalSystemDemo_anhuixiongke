...
namespace MedicalSystem.Anes.Document.Documents
{
    ...
    public partial class DocumentTemplet : BaseView
    {
        ...
        {
            _documentTempletDataTable = CommonService.ClientInstance.GetDocumentTemplet();
            ...
            if (_isApply)
            {
                ...
                else
                {
                    ...
                    foreach (MED_DOCUMENT_TEMPLET row in _documentTempletDataTable)
                    {
                        AppendNode(row);
                    }
                }
            ...
            else
            {
                ...
                if (ExtendAppContext.Current.CustomSettingContext.CustomTempletFlagNames.Count > 0)
                {
                    foreach (string flagName in ExtendAppContext.Current.CustomSettingContext.CustomTempletFlagNames)
                    {
                        bool find = false;
        ...
        {
            //if (false) // (!AccessControl.CheckModifyRight("模板管理"))
            //{
        ...
        public override bool Save()
        {
            ...
            if (prows != null && prows.Count > 0)
            {
                ...
                if (CommonService.ClientInstance.SaveDocumentTemplet(prow))
                {
                    b = true;
                    btnSave.Enabled = false;
                }
