...
SELECT
       ...
       FROM
       (SELECT * FROM V_OPERATION_SCHEDULE  WHERE convert(nvarchar(10),SCHEDULED_DATE_TIME,120) = convert(nvarchar(10),GETDATE(),120) AND STATE = 2) A , V_PAT_MASTER_INDEX C,
       V_OPERATING_ROOM D
       WHERE
       A.PATIENT_ID = C.PATIENT_ID              AND
       A.OPERATING_ROOM = D.DEPT_CODE AND A.OPERATING_ROOM_NO = D.ROOM_NO
UNION
SELECT
       A.PATIENT_ID,                                       --病人ID (住院号)
       A.VISIT_ID,                                         --本次住院标示
      ...
      ISNULL((SELECT V_DEPT_DICT.DEPT_NAME   FROM V_DEPT_DICT WHERE V_DEPT_DICT.DEPT_CODE = A.OPERATING_DEPT ),A.OPERATING_DEPT ) OPERATING_DEPT,
	   ...
	   A.INFECTED_IND,                                     --感染标志
       A.OPERATION_POSITION,                               --体位
       CASE A.OPER_STATUS WHEN  5 THEN '术中' WHEN 10 THEN '术中' WHEN 15 THEN '术中' WHEN 25 THEN '术中' WHEN 30 THEN '术中' WHEN 35 THEN '术中' WHEN 40 THEN '复苏室' WHEN 45 THEN '复苏室' END AS TYPE,
       A.OPER_STATUS,
       ...
       OPER_STATUS < 50 ) A , V_ANESTHESIA_PLAN C , V_PAT_MASTER_INDEX D,V_OPERATING_ROOM E
       WHERE
       A.PATIENT_ID = D.PATIENT_ID                            AND
       A.PATIENT_ID = C.PATIENT_ID                            AND
       A.VISIT_ID = C.VISIT_ID                                AND
       A.OPER_ID = C.OPER_ID                                  AND
       A.OPERATING_ROOM = E.DEPT_CODE AND A.OPERATING_ROOM_NO = E.ROOM_NO
;
