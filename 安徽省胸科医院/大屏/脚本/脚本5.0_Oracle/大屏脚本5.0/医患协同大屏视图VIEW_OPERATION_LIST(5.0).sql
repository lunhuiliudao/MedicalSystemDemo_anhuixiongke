--先删除原来有的MEDCOMM下的视图
DROP VIEW MEDCOMM.VIEW_OPERATION_LIST
...
SELECT
  ...
  A.RADIATE_IND,                                      --放射标志
   A.INFECTED_IND,                                     --感染标志
       ...
       FROM
       (SELECT * FROM V_OPERATION_SCHEDULE  WHERE TO_CHAR(SCHEDULED_DATE_TIME,'yyyyMMdd') = TO_CHAR(SYSDATE ,'yyyyMMdd')  AND STATE = 2) A 
...
SELECT
  ...
  A.RADIATE_IND,                                      --放射标志
   A.INFECTED_IND,                                     --感染标志
       ...
       DECODE( A.OPER_STATUS, 5,'术中',10,'术中',15,'术中',25,'术中',30,'术中',35,'术后',40,'复苏',45,'复苏') END  TYPE,
       A.OPER_STATUS,
