    ...
    >
      <el-form :model="formData" :rules="rules" ref="formView" :inline="true">
        ...
        <el-form-item label="抗生素等级">
          <el-select v-model="formData.ANTIBIOTIC" filterable placeholder="请选择" style="width:175px">
            <el-option
              style="font-size:14px;padding:0px 5px;"
              v-for="(item,index) in antibioticTypeList"
              :key="index"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" class="drugEditDivSave" @click="onSubmit('formView')">保 存</el-button>&nbsp;&nbsp;&nbsp;&nbsp;
        <el-button type="primary" class="drugEditDivSave" @click="dialogEditVisible = false">取 消</el-button>
      </div>
    </el-dialog>
    <div id="conditionView">
      <div style="margin:10px 10px 0px;height:26px">
        ...
        <div style="width:200px;float:left">
          ...
          <!-- <el-select v-model="itemName" clearable filterable remote placeholder="请选择药品名称" :remote-method="itemNameRemoteMethod">
            <el-option v-for="(item, index) in itemNameList" :key="index" :label="item.EVENT_ITEM_NAME" :value="item.EVENT_ITEM_CODE">
            </el-option>
          </el-select>-->
        </div>
        <div>
          <el-button type="primary" class="drugEditDivAdd" @click="searchData(1)">查询</el-button>
          <el-button type="primary" class="drugEditDivAdd" @click="addData">新增</el-button>
        </div>
      </div>
    </div>
    <div style="background-color: #fff;min-height:600px;">
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="ANTIBIOTIC" header-align="center" align="center" label="抗生素等级"></el-table-column>
          <el-table-column label="操作" align="center" fixed="right" min-width="120px">
            <template slot-scope="scope">
              <span class="czCss">
                <el-button
                  size="small"
                  class="drugEditDivEdit"
                  @click="editData(scope.$index, scope.row)"
                >编辑</el-button>
                <el-button
                  size="small"
                  class="drugEditDivDelete"
                  type="danger"
...
<script>
import DrugEdit from './DrugEdit.js'
...
.drugEditDiv .el-input__icon {
  line-height: 25px;
}
.drugEditDiv .el-button {
  display: inline-block;
  line-height: 1;
  white-space: nowrap;
  cursor: pointer;
...
.drugEditDiv .el-table th {
  padding: 6px 0;
}

.drugEditDiv .el-table td {
  padding: 4px 0;
}

.drugEditDiv .el-input-number {
  line-height: 24px;
}

.drugEditDiv .el-select-dropdown__list {
  padding: 0px 10px;
}

.drugEditDiv ::-webkit-input-placeholder {
  font-style: italic;
}

@media screen and (min-aspect-ratio: 1001/1000) {
  /* 横屏 */
  .drugEditDiv .el-form-item__label {
    font-size: 12px;
