...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        EVENT_ITEM_NAME: [
          { required: true, message: '请输入输血名称', trigger: 'blur' }
        ]
      },
  ...
  methods: {
    ...
    AdministratorDict () {
      var _this = this
      // this.$ajax.get('/Api/PlatformCommon/GetAdministrationDict')
    ...
    getEventAttrDict () {
      ...
      BasicConfigApi.GetAnesInputDictByClass({
        ...
        .then(function (respose) {
          _this.eventAttrList = respose.data.Data
        })
        .catch(error => {
          console.log(error)
        })
