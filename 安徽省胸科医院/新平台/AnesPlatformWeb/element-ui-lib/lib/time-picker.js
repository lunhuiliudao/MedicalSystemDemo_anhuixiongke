...
/* harmony default export */ var pickervue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    handleKeydown: function handleKeydown(event) {
      ...
      // 按键为数字或者分隔符
      if (keyCode > 47 && keyCode < 58 || keyCode > 95 && keyCode < 106 || keyCode === 229) {
        // 229为输入法中文下keyCode BUG
        if (this.type === 'datetime') {
          var keyDate = [4, 7];
          var keyTime = [13, 16];
          var value = event.target.value;
          var selectionStart = event.target.selectionStart; // 光标位置
          var dateReplaceChat = this.format ? this.format.substring(4, 5) : '-';
          var timeReplaceChat = this.format ? this.format.substring(13, 14) : ':';
          var splice = this.format ? this.format.substring(10, 11) : ' ';

          // 光标在末尾
          if (selectionStart === value.length) {
            if (keyDate.indexOf(value.length) > -1) {
              event.target.value = value + dateReplaceChat;
            } else if (value.length === 10) {
              event.target.value = value + splice;
            } else if (keyTime.indexOf(value.length) > -1) {
              event.target.value = value + timeReplaceChat;
            }
          }

          // 修改时光标在中间
          if (event.target.selectionStart < value.length) {
            var nextChat = value.substring(selectionStart, selectionStart + 1);
            if (keyDate.indexOf(selectionStart) > -1) {
              if (nextChat === dateReplaceChat) {
                event.target.value = value.substring(0, selectionStart + 1) + value.substring(selectionStart + 2, value.length);
                event.target.setSelectionRange(selectionStart + 1, selectionStart + 1);
              } else {
                event.target.value = value.substring(0, selectionStart) + dateReplaceChat + value.substring(selectionStart + 1, value.length);
                event.target.setSelectionRange(selectionStart + 1, selectionStart + 1);
              }
              ...
              if (nextChat === splice) {
                ...
                event.target.value = value.substring(0, selectionStart) + splice + value.substring(selectionStart + 1, value.length);
                event.target.setSelectionRange(selectionStart + 1, selectionStart + 1);
              }
            } else if (keyTime.indexOf(selectionStart) > -1) {
              if (nextChat === timeReplaceChat) {
                event.target.value = value.substring(0, selectionStart + 1) + value.substring(selectionStart + 2, value.length);
                event.target.setSelectionRange(selectionStart + 1, selectionStart + 1);
              } else {
                event.target.value = value.substring(0, selectionStart) + timeReplaceChat + value.substring(selectionStart + 1, value.length);
                event.target.setSelectionRange(selectionStart + 1, selectionStart + 1);
              }
            } else {
              if (nextChat !== dateReplaceChat && nextChat !== splice && nextChat !== timeReplaceChat) {
                event.target.value = value.substring(0, selectionStart) + value.substring(selectionStart + 1, value.length);
          ...
          var _selectionStart = event.target.selectionStart; // 光标位置
          var _dateReplaceChat = this.format ? this.format.substring(4, 5) : '-';

          // 光标在末尾
          if (_selectionStart === _value.length) {
            if (_keyDate.indexOf(_value.length) > -1) {
              event.target.value = _value + _dateReplaceChat;
            }
          }

          // 修改时光标在中间
          if (event.target.selectionStart < _value.length) {
            var _nextChat = _value.substring(_selectionStart, _selectionStart + 1);
            if (_keyDate.indexOf(_selectionStart) > -1) {
              if (_nextChat === _dateReplaceChat) {
                event.target.value = _value.substring(0, _selectionStart + 1) + _value.substring(_selectionStart + 2, _value.length);
                event.target.setSelectionRange(_selectionStart + 1, _selectionStart + 1);
              } else {
                event.target.value = _value.substring(0, _selectionStart) + _dateReplaceChat + _value.substring(_selectionStart + 1, _value.length);
                event.target.setSelectionRange(_selectionStart + 1, _selectionStart + 1);
              }
            } else {
              if (_nextChat !== _dateReplaceChat) {
                event.target.value = _value.substring(0, _selectionStart) + _value.substring(_selectionStart + 1, _value.length);
                event.target.setSelectionRange(_selectionStart, _selectionStart);
              }
            }
          }
        }
