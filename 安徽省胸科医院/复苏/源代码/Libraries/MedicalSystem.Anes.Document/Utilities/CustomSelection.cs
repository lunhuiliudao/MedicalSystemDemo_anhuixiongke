...
namespace MedicalSystem.Anes.Document.Utilities
{
    ...
    public class CustomSelection
    {
                ...
                if (_displayNames != null && _displayNames.Length > 0)
                {
                    try
                    {
                        ...
                        if (type.Equals(typeof(DataRow)))
                        {
                            ...
                            if (((DataRow)obj).Table.Columns.Contains("INPUT_CODE"))
                            {
                                inputCode = ((DataRow)obj)["INPUT_CODE"].ToString();
                            }
                            _listItems.Add(new CustomSelectionListItem(index, ((DataRow)obj)[_displayNames[0]].ToString(), rowArray, inputCode));
                        }
                        else
                        {
                            // 原先的方法从list转datarow肯定报错...
                            object inputCodeObj = type.GetProperty("INPUT_CODE").GetValue(obj, null);
                            string inputCode = inputCodeObj == null ? string.Empty : inputCodeObj.ToString();

                            //if (((DataRow)obj).Table.Columns.Contains("INPUT_CODE"))
                            //{
                            //    inputCode = ((DataRow)obj)["INPUT_CODE"].ToString();
                            //}
                            _listItems.Add(new CustomSelectionListItem(index, type.GetProperty(_displayNames[0]).GetValue(obj, null).ToString(), rowArray, inputCode));
                        }
                    }
                    catch (Exception)
                    {
        ...
        private static void SetDataGridViewList(DataGridView dataGridView)
        {
            ...
            {
                if (!string.IsNullOrEmpty(_filtString) && !item.InputCode.ToLower().Contains(_filtString.ToLower()))
                {
                    continue;
                }
