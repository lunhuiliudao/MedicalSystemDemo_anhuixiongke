...
namespace MedicalSystem.Anes.Document.Utilities
{
    ...
    {
        static Configuration configByUserLevel = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
        ...
        public static string Read(string key)
        {
            try
            {
                ...
                string value = configByUserLevel.AppSettings.Settings[key].Value;
                return value;
            }
            catch (Exception ex)
            {
        ...
        public static void Save(string key, string value)
        {
            ...
            //Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            configByUserLevel.AppSettings.Settings.Remove(key);
            configByUserLevel.AppSettings.Settings.Add(key, value);
            configByUserLevel.Save();
        }
        ...
        public static ConnectionStringSettingsCollection ConnectionStringSettingsCollections
        {
            get
            {
                ...
                ConnectionStringSettingsCollection connectionStringSettingsCollection = configByUserLevel.ConnectionStrings.ConnectionStrings;
        ...
        public static bool SaveConnectionStringSettings(ConnectionStringSettingsCollection connectionStringSettingsCollections)
        {
            ...
            {
                ConnectionStringSettings connectionStringSettings1 = configByUserLevel.ConnectionStrings.ConnectionStrings[connectionStringSettings.Name];
                if (connectionStringSettings1 == null)
                {
                    configByUserLevel.ConnectionStrings.ConnectionStrings.Add(connectionStringSettings);
                    result = true;
                }
                else if (!IsSame(connectionStringSettings1, connectionStringSettings))
                {
                    configByUserLevel.ConnectionStrings.ConnectionStrings.Remove(connectionStringSettings1);
                    configByUserLevel.ConnectionStrings.ConnectionStrings.Add(connectionStringSettings);
                    result = true;
                }
            }
            if (result)
            {
                ProtectSection(configByUserLevel.ConnectionStrings);
                configByUserLevel.Save();
            }
            return result;
        }
