...
namespace MedicalSystem.Anes.Client.CustomProject
{
    ...
    public partial class BloodGasSelector : UserControl
    {
        ...
        {
            string itemText = "静脉";
            if (!string.IsNullOrEmpty(row.NURSE_MEMO1) && !string.IsNullOrEmpty(row.NURSE_MEMO1.Trim()))
            {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            else if (listView2.Items.Count > 0 && listView2.SelectedItems.Count == 1)
            {
                SelectedDetailID = listView2.SelectedItems[0].Tag.ToString();
                ParentForm.DialogResult = DialogResult.OK;
            }
        }
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            {
                List<MED_BLOOD_GAS_MASTER> bloodGasMasterDataTable = operationInfoRepository.GetBloodGasMasterListByID(listView1.FocusedItem.Tag.ToString()).Data;
                if (bloodGasMasterDataTable != null && bloodGasMasterDataTable.Count == 1)
                {
                    ...
                    if (operationInfoRepository.SaveBloodGasMaster(bloodGasMasterDataTable).Data > 0)
                    {
                        resetListView();
                    }
                }
        ...
        private void toolStripMenuItem2_Click(object sender, EventArgs e)
        {
            ...
            {
                List<MED_BLOOD_GAS_MASTER> bloodGasMasterDataTable = operationInfoRepository.GetBloodGasMasterListByID(listView2.FocusedItem.Tag.ToString()).Data;
                if (bloodGasMasterDataTable != null && bloodGasMasterDataTable.Count == 1)
                {
                    bloodGasMasterDataTable[0].PATIENT_ID = "0";
                    bloodGasMasterDataTable[0].VISIT_ID = 0;
