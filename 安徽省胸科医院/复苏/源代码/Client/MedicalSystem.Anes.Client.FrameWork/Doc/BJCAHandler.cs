        ...
        private void OnCustomDelete(object sender, EventArgs e)
        {
            try
            {
                if (sender is BJCAControl)
                {
                    ...
                    List<MED_BJCA_SIGN> caList = CommonService.GetBjcaSignList(patientId, visitId, operId);

                    List<MED_BJCA_SIGN> temp = caList.Where(x => x.CONTROLID.Equals(ca.Name) &&
                                                                 x.SIGNNAME.Equals(ca.CertName) &&
                                                                 x.CERTSN.Equals(ca.CertSN) &&
                                                                 x.SIGNDOCNAME.Equals(this.DocName)).ToList();

                    if (temp.Count > 0)
                    {
                        CommonService.UpdateBjcaSignList(temp);
                        ca.ClearPic();
            ...
            {
                MedicalSystem.Anes.Core.Log.LogHelper.WriteErrLog("删除签名图片异常", ex);
