...
namespace BLL
{
    public class BMedOrders:HospitalBase
    {
        ...
        public override string PreDataBase(InitDocare.Config2 config, Init.ParamInputDomain domain)
        {
            try
            {
                InitDocare.LogA.Debug("步医嘱:start:" + domain.Patient.StartDate + " stopdatetie:" + domain.Patient.StopDate);
                ...
                foreach (MedIfHisViewSql2 temp in sql)
                {
                    ...
                    para[2].Value = vspat.HisPatientId;   //todo 多个参数动态怎么解决
                    para[3].Value = vspat.HisInpNo;

                    foreach (SqlParameter sp in para)
                    {
                       InitDocare.LogA.Debug("sp:" + sp.ParameterName + " value:" + sp.Value);
                    }
        ...
        public override string PreWebServices(InitDocare.Config2 config, Init.ParamInputDomain domain)
        {
            ...
            InitDocare.LogDALA.Debug(" domain.Patient.InpNo:" + domain.Patient.InpNo);
        ...
        public override string DoExecute(InitDocare.Config2 config, Init.ParamInputDomain domain, MedicalSytem.Soft.Model.ModelBase obj)
        {
            try
            {
                ...
                MedicalSytem.Soft.Model.MedVsHisOrders medVsHisOrdersTemp = SelectMedVsHisOrdersHis(order.OrderNo.ToString(), order.HisOrderSubNo, new DateTime(2011, 1, 1));
                ...
                if (order.OrderSubNo > 1)
                {
                    ...
                    {
                        order.StartDateTime = orderSubTemp.StartDateTime;
                        order.StopDateTime = orderSubTemp.StopDateTime;
